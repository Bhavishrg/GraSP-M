    ...
    do
        ...
        do
            ...
            if [ $party -eq 1 ]; then
                ...
                ./benchmarks/reconstruction -p $party --localhost -l 100.0 -i 10 -n $players --use-pking true 2>&1 | cat > $log &
            else
                # ./benchmarks/equality -p $party --localhost -l 100.0 -i 30 -n $players 2>&1 | cat > $log &
                ./benchmarks/reconstruction -p $party --localhost -l 100.0 -i 10 -n $players --use-pking true 2>&1 | cat > $log &
            fi
        ...
        ./benchmarks/reconstruction  -p 0 --localhost -l 100.0 -i 10 -n $players --use-pking true 2>&1 | cat > $tplog & codes[0]=$!
