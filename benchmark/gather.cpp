...
common::utils::Circuit<Ring> generateCircuit(int nP, int pid, size_t vec_size, size_t num_groups) {
    ...
    std::cout << "Generating gather circuit with vec_size=" << vec_size 
    ...
    // Use utility function to add gather sub-circuit
    auto data_values_diff = addSubCircGather(circ, position_map_shares, data_values, num_groups, permutation);

    // Set outputs
    for (size_t i = 0; i < vec_size; ++i) {
        circ.setAsOutput(data_values_diff[i]);
    }
...
void benchmark(const bpo::variables_map& opts) {
    ...
    if (!input_wires.empty()) {
        ...
        // Create a test position map
        std::vector<Ring> position_map_values(vec_size);
        position_map_values[0] = static_cast<Ring>(3);
        position_map_values[1] = static_cast<Ring>(4);
        position_map_values[2] = static_cast<Ring>(5);
        position_map_values[3] = static_cast<Ring>(0);
        position_map_values[4] = static_cast<Ring>(6);
        position_map_values[5] = static_cast<Ring>(1);
        position_map_values[6] = static_cast<Ring>(7);
        position_map_values[7] = static_cast<Ring>(8);
        position_map_values[8] = static_cast<Ring>(2);
        position_map_values[9] = static_cast<Ring>(9);
    ...
    std::cout << "  ✓ GATHER COMPLETE - Computed prefix sum, shuffled, rewired, and computed differences" << std::endl;
    std::cout << "============================\n" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    bpo::options_description cmdline("Benchmark online phase for gather circuit.");
