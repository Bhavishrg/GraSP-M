...
common::utils::Circuit<Field> generateCircuit(int nP, int pid, int vec_size) {
    ...
    // Create two separate sets of inputs for two parallel permAndSh gates
    ...
    // Create two permutations (one per party)
    ...
    // First permAndSh gate
    auto outputs1 = circ.addMGate(common::utils::GateType::kPermAndSh, input_vector1, permutations1, owner1);
    
    // Second permAndSh gate (in parallel)
    auto outputs2 = circ.addMGate(common::utils::GateType::kPermAndSh, input_vector2, permutations2, owner2);
...
void benchmark(const bpo::variables_map& opts) {
    ...
    std::cout << "\n=== PERM AND SHARE RESULT ===" << std::endl;
    std::cout << "Party " << pid << " output:" << std::endl;
    std::cout << "  Number of outputs: " << outputs.size() << std::endl;
    std::cout << "  Expected: " << (vec_size * 2) << " outputs (2 permAndSh gates)" << std::endl;
    
    if (outputs.size() >= static_cast<size_t>(vec_size*2)) {
        std::cout << "\n  First permAndSh gate outputs:" << std::endl;
        for (int i = 0; i < vec_size; ++i) {
            std::cout << "    Output[" << i << "] = " << outputs[i] << std::endl;
        }
        
        std::cout << "\n  Second permAndSh gate outputs:" << std::endl;
        for (int i = 0; i < vec_size; ++i) {
            std::cout << "    Output[" << i << "] = " << outputs[vec_size + i] << std::endl;
        }
    }
    std::cout << "============================\n" << std::endl;
...
bpo::options_description programOptions() {
    ...
    desc.add_options()
        ...
        ("vec-size", bpo::value<int>()->default_value(4), "Size of vectors for permAndSh gates.");
  return desc;
}
// clang-format on
...
int main(int argc, char* argv[]) {
    ...
    bpo::options_description cmdline("Benchmark online phase for permAndSh gates.");
...
// usage ./../run.sh permsh --num-parties 3 --vec-size 4
